...
integer                      :: start_keep, end_keep
    ...
    if(size(manual).eq.0)then
       ...
       write(*,'(g0)')'   fan -e TOPIC'
       write(*,'(g0)')'or search the entire manual:'
       write(*,'(g0)')'   fan manual -i -e TOPIC'
       stop 1
    else
...
integer :: i,j,k
   ...
   if(demo)then
    ...
    if(size(newsection).eq.0)then
       write(*,*)'!<ERROR> *fan* standard demo code format not found for ',trim(topics(i))
       section=['']
    else
       section=newsection
...
'NAME                                                                            ',&
'    fan(1f) - [FUNIX] output descriptions of Fortran intrinsics                 ',&
'    (LICENSE:PD)                                                                ',&
'                                                                                ',&
'SYNOPSIS                                                                        ',&
'    fan NAME(s) [[-ignorecase][--regex Regular_Expression]]|[-topic_only]       ',&
'                [--color][--demo]                                               ',&
'                                                                                ',&
'    fan [ --help| --version]                                                    ',&
'                                                                                ',&
'DESCRIPTION                                                                     ',&
'   fan(1) prints descriptions of Fortran intrinsics using the                   ',&
'   M_intrinsics(3f) module as simple flat text, but formatted in the            ',&
'   txt2man(1) markdown format so it can easily generate man-pages on ULS        ',&
'   (Unix-Like Systems).                                                         ',&
'                                                                                ',&
'OPTIONS                                                                         ',&
'  TOPIC(s)          Topic names. A list of Fortran intrinsic names or the       ',&
'                    special names "toc" and "manual", which generate a          ',&
'                    table of contents and the entire set of documents           ',&
'                    respectively. The default is "toc" and to ignore case.      ',&
...
'  --demo,-d         extra first demo program found for a topic, starting with   ',&
...
'                    to display colors. **PRELIMINARY ALPHA VERSION**            ',&
...
'                                                                                ',&
'   # list the topic "scan" if found and lines containing "scan" from the        ',&
'   # entire manual, prefixing the lines with the section name ignore case.      ',&
'   fan -e scan -i manual                                                        ',&
'   fan -d verify >demo_verify.f90 # get sample program to try VERIFY(3f)        ',&
'']
