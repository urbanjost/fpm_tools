...
!!    bytes or integer(kind=1) using the transfer intrinsic:
!!
...
!! end program demo_M_sha3
  ...
  type sha3_state
     ...
     integer(kind=1), dimension(:), pointer :: buffer
...
! stdout, along with the file name. d is the digest size in bits (224,256,384,512)
  ...
  integer(kind=1), dimension(d/8)              :: digest
...
! absorbs the content of buffer into the sponge.
  ...
  integer :: i, k, a, b
  integer(kind=1), dimension(LANE) :: bytes
...
use M_strings, only : setbits8
  ...
  type(sha3_state),         intent(inout) :: state
  integer(kind=1), dimension(:), intent(out)   :: digest

  integer :: i, j
  integer(kind=1), dimension(25*LANE) :: string
  ...
  if ( i == 0 ) then
     ...
     !GFORTRAN_BUG!state%buffer( state%r/8 ) = transfer(int(b'10000110',kind=1),state%buffer(1))
     state%buffer( state%r/8 ) = transfer(bug,state%buffer(1))
  else
     state%buffer( state%bufsize + 1 ) = transfer(int(b'00000110',kind=1),state%buffer(1))
     state%buffer( state%bufsize + 2 : state%r/8 - 1 ) = 0_1
     bug=setbits8('10000000')
     !GFORTRAN_BUG!state%buffer( state%r/8 ) = transfer(int(b'10000000',kind=1),state%buffer(1))
     state%buffer( state%r/8 ) = transfer(bug,state%buffer(1))
  endif
...
! we assume that d is a multiple of 8
  ...
  integer(kind=1), dimension(d/8) :: sha3
...
function sha3_keccak( M, d, c )
  ...
  integer(kind=1), dimension(d/8) :: sha3_keccak
...
function sha3_sponge( M, d, r )
  ...
  integer(kind=1), dimension(d/8) :: sha3_sponge

  integer :: i, c, n, j, k

  integer(kind=1), dimension(25*LANE) :: S        ! state, as a string
  integer(kind=1), dimension(r/8)     :: padding
  ...
  if ( d == 224 .or. d == 256 .or. d == 384 .or. d == 512 ) then
     ...
     if ( i == 0 ) then
        ...
        enddo
        padding(r/8) = transfer(int(b'01100001',kind=1),padding(1))
        n = (size(M) - (r/8-1)) / (r/8)
     else
        padding = 0_int8
        do j = 1, r/8-1-i
        ...
        enddo
        padding(r/8-i) = transfer(int(b'01100000',kind=1),padding(1))
        padding(r/8)   = transfer(int(b'00000001',kind=1),padding(1))
...
function sha3_keccak_p( S, b, nr )
  ...
  integer(kind=1), dimension(:), intent(in) :: S  ! input "string"
  integer,                  intent(in) :: b  ! size of input, in bits
  integer,                  intent(in) :: nr ! number of rounds
  integer(kind=1), dimension(b/8) :: sha3_keccak_p
...
! reverses the order of the bits in byte b
  ...
  !integer(kind=int8),parameter :: Z33=int(z'33',kind=int8)
  !integer(kind=int8),parameter :: Z55=int(z'55',kind=int8)
  !integer(kind=int8),parameter :: ZAA=int(z'AA',kind=int8)
  !integer(kind=int8),parameter :: ZCC=int(z'CC',kind=int8)
  !integer(kind=int8),parameter :: ZF0=int(z'F0',kind=int8)
  !===================
...
! in the order  A(1,1)  A(2,1)  A(3,1)  A(4,1)  A(5,1)   A(1,2) ... A(5,5)
  ...
  integer(kind=1), dimension(:), intent(in) :: S  ! input "string" as a list of bytes
  integer(kind=int64), dimension(5,5) :: sha3_string2state

  integer(kind=1), dimension(8) :: reve
  integer :: x, y, z, i
...
! convert a state S to a string (array) of sz bytes
  ...
  integer(kind=1), dimension(sz) :: sha3_state2string2  ! input "string" as a list of bytes
...
!================================================================================
  integer(kind=1), dimension(512/8) :: digest
  integer(kind=1), dimension(:), allocatable :: buffer
  type(sha3_state) :: S
...
!================================================================================
  ...
  integer(kind=1), dimension(224/8) :: digest
  integer(kind=1), dimension(:), allocatable :: buffer
  type(sha3_state) :: S
