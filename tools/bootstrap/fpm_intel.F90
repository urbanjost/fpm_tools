...
subroutine split(input_line,array,delimiters,order,nulls)
    ...
    idlim=len(dlim)                                                   ! dlim a lot of blanks on some machines if dlim is a big strin

    if(present(order))then
    ordr=lower(adjustl(order)); else; ordr='sequential'
    endif ! decide on value for optional ORDER paramete
    if(present(nulls))then
    nlls=lower(adjustl(nulls)); else; nlls='ignore'
    ...
    case ('reverse','right')
    ii=ireturn
    iiii=-1                ! last to first
    case default
    ii=1
    iiii=1                 ! first to last
    end select
...
integer                              :: i
   ...
   left_local=left   ; else ; left_local=''
   endif
   if(present(right))then
   right_local=right ; else ; right_local=''
   endif
   string=''
...
module tomlf_datetime
   ...
   type :: toml_time
      ...
      !NVFORTRAN BUG!character(len=:), allocatable :: zone
      character(len=:), allocatable :: zone
      !NVFORTRAN!character(len=5), allocatable :: zone
   contains
...
function toml_raw_to_timestamp(raw, timestamp) result(stat)
   ...
   if (toml_raw_verify_time(raw(first:))) then
      !NVFORTRAN_BUG!timestamp%time = toml_time()
...
subroutine toml_normalize_string(str)
   ...
   do i = 1, len(str)
      ...
      if (escape) then
         ...
         select case(ch)
         case('b')
         tmp = tmp // TOML_BACKSPACE
         case('t')
         tmp = tmp // TOML_TABULATOR
         case('n')
         tmp = tmp // TOML_NEWLINE
         case('f')
         tmp = tmp // TOML_FORMFEED
         case('r')
         tmp = tmp // TOML_CARRIAGE_RETURN
         case('"', '\')
         tmp = tmp // ch
...
!>>>>> ././src/tomlf_structure_vector.f90
...
module tomlf_type
   ...
   use tomlf_type_table, only : new_table, new
   use tomlf_type_value, only : toml_value, toml_visitor, toml_key
   use tomlf_type_table, only : toml_table
   implicit none
   private

   public :: toml_value, toml_visitor, toml_array, toml_keyval
   public :: toml_key
...
module tomlf_build_keyval
   ...
   use tomlf_error, only : toml_stat
   use tomlf_type_table, only : toml_table
   use tomlf_type, only : toml_value, toml_array, toml_keyval, &
...
module tomlf_build_merge
   use tomlf_constants, only : tfc
   use tomlf_type_table, only : toml_table
   use tomlf_type, only : toml_array, toml_keyval, toml_value, &
      & toml_key, len
   implicit none
   private
...
module tomlf_de_tokenizer
   ...
   use tomlf_utils
   use tomlf_type_table, only: toml_table
   use tomlf_type, only : toml_value, toml_key, toml_array, &
...
module tomlf_ser
   ...
   use tomlf_type_table, only : toml_table
   use tomlf_type, only : toml_value, toml_visitor, toml_key, &
...
subroutine next_token(de, dot_is_token)
   ...
   do while(len(ptr) > 0)
      ...
      case(',')
      de%tok = new_token(toml_tokentype%comma, ptr, 1)
      return
      case('=')
      de%tok = new_token(toml_tokentype%equal, ptr, 1)
      return
      case('{')
      de%tok = new_token(toml_tokentype%lbrace, ptr, 1)
      return
      case('}')
      de%tok = new_token(toml_tokentype%rbrace, ptr, 1)
      return
      case('[')
      de%tok = new_token(toml_tokentype%lbracket, ptr, 1)
      return
      case(']')
      de%tok = new_token(toml_tokentype%rbracket, ptr, 1)
      return
      case(TOML_NEWLINE)
      de%tok = new_token(toml_tokentype%newline, ptr, 1)
      return
      case(' ', char(9))
...
module tomlf
   ...
   use tomlf_type_table, only : toml_table
   use tomlf_type, only : toml_array, toml_key, is_array_of_tables, &
...
module fpm_toml
    ...
    public :: toml_array, toml_key, toml_stat, get_value, set_value
...
!>>>>> ././src/fpm_manifest_dependency.f90
...
!>>>>> ././src/fpm_manifest_executable.f90
            ...
            if (module_name == targets(k)%ptr%source%modules_provided(l)%s) then
                ...
                case default
                    if (present(include_dir)) then
                        if (index(dirname(targets(k)%ptr%source%file_name), include_dir) == 1) then ! source file is within the incl
                            target_ptr => targets(k)%ptr
                            exit
                        end if
                    end if
...
interface  get_args
module  procedure  get_anyarray_d
end interface  ! any size array
interface  get_args
module  procedure  get_anyarray_i
end interface  ! any size array
interface  get_args
module  procedure  get_anyarray_r
end interface  ! any size array
interface  get_args
module  procedure  get_anyarray_x
end interface  ! any size array
interface  get_args
module  procedure  get_anyarray_c
end interface  ! any size array and any length
interface  get_args
module  procedure  get_anyarray_l
end interface  ! any size array

! return scalars
interface  get_args
module  procedure  get_scalar_d
end interface
interface  get_args
module  procedure  get_scalar_i
end interface
interface  get_args
module  procedure  get_scalar_real
end interface
interface  get_args
module  procedure  get_scalar_complex
end interface
interface  get_args
module  procedure  get_scalar_logical
end interface
interface  get_args
module  procedure  get_scalar_anylength_c
end interface  ! any length
! multiple scalars
interface  get_args
module  procedure  many_args
end  interface
!==================================================================================================================================
...
module procedure get_fixedarray_class
end interface ! any length, fixed size array
...
module  procedure  get_args_fixed_length_a_array
end interface  ! fixed length any size array
interface   get_args_fixed_length
module  procedure  get_args_fixed_length_scalar_c
end interface       ! fixed length
!===================================================================================================================================
!intrinsic findloc
!===================================================================================================================================
...
! convenience functions
interface cgets
module procedure cgs, cg
end interface
interface dgets
module procedure dgs, dg
end interface
interface igets
module procedure igs, ig
end interface
interface lgets
module procedure lgs, lg
end interface
interface rgets
module procedure rgs, rg
end interface
interface sgets
module procedure sgs, sg
end interface
!-----------------------------------------------------------------------------------------------------------------------------------
contains
!===================================================================================================================================
...
character(len=*),intent(in),optional :: delimiters
   select type(generic)
    ...
    call get_fixedarray_fixed_length_c(keyword,generic,delimiters)
    type is (integer)
    call get_fixedarray_i(keyword,generic,delimiters)
    type is (real)
    call get_fixedarray_r(keyword,generic,delimiters)
    type is (complex)
    call get_fixed_size_complex(keyword,generic,delimiters)
    type is (real(kind=dp))
    call get_fixedarray_d(keyword,generic,delimiters)
    type is (logical)
    call get_fixedarray_l(keyword,generic,delimiters)
    class default
...
integer                      :: iichar                     ! point to first character of word unless first character is "."
   ...
   if(size(carray).gt.0)then                                  ! if not a null string
      ...
      do i=1,size(carray)
         ...
         case('T','Y',' ')
         larray(i)=.true.               ! anything starting with "T" or "Y" or a blank is TRUE (true,yes,...)
         case('F','N')
         larray(i)=.false.              ! assume this is false or no
         case default
...
class(*),intent(in) :: generic
   ...
   select type(generic)
      ...
      type is (complex)
      write(line(istart:),'("(",1pg0,",",1pg0,")")') generic
   end select
   if(debug_m_cli2)write(*,gen)'<DEBUG>PRINT_GENERIC:START'
...
integer :: i
   select type(generic)
      type is (integer(kind=int8))
      write(line(istart:),'("[",*(i0,1x))') generic
      type is (integer(kind=int16))
      write(line(istart:),'("[",*(i0,1x))') generic
      type is (integer(kind=int32))
      write(line(istart:),'("[",*(i0,1x))') generic
      ...
      write(line(istart:),'("[",*(i0,1x))') generic
      type is (real(kind=real32))
      write(line(istart:),'("[",*(1pg0,1x))') generic
      type is (real(kind=real64))
      write(line(istart:),'("[",*(1pg0,1x))') generic
      ...
      type is (logical)
      write(line(istart:),'("[",*(l1,1x))') generic
      type is (character(len=*))
         if(debug_m_cli2)write(*,gen)'<DEBUG>PRINT_GENERIC:CHARACTER'
         if(debug_m_cli2)write(*,gen)'<DEBUG>PRINT_GENERIC:ISTART:',istart
         write(line(istart:),'("[",:*("""",a,"""",1x))') (trim(generic(i)),i=1,size(generic))
      type is (complex)
      write(line(istart:),'("[",*("(",1pg0,",",1pg0,")",1x))') generic
      class default
...
call get_args(n,iget)
end function iget
function rget(n)
real                         :: rget
character(len=*),intent(in) :: n
call get_args(n,rget)
end function rget
function dget(n)
real(kind=dp)                :: dget
character(len=*),intent(in) :: n
call get_args(n,dget)
end function dget
function sget(n)
character(len=:),allocatable :: sget
character(len=*),intent(in) :: n
call get_args(n,sget)
end function sget
function cget(n)
complex                      :: cget
character(len=*),intent(in) :: n
call get_args(n,cget)
end function cget
function lget(n)
logical                      :: lget
character(len=*),intent(in) :: n
call get_args(n,lget)
end function lget

function igs(n)
integer,allocatable          :: igs(:)
character(len=*),intent(in) :: n
call get_args(n,igs)
end function igs
function rgs(n)
real,allocatable             :: rgs(:)
character(len=*),intent(in) :: n
call get_args(n,rgs)
end function rgs
function dgs(n)
real(kind=dp),allocatable    :: dgs(:)
character(len=*),intent(in) :: n
call get_args(n,dgs)
end function dgs
function sgs(n)
...
function cgs(n)
complex,allocatable          :: cgs(:)
character(len=*),intent(in) :: n
call get_args(n,cgs)
end function cgs
function lgs(n)
logical,allocatable          :: lgs(:)
character(len=*),intent(in) :: n
call get_args(n,lgs)
end function lgs
!===================================================================================================================================
...
character,allocatable :: hold
   ...
   do i=1,size(lg)
      ...
      case('T','Y',' ')
      lg(i)=.true.          ! anything starting with "T" or "Y" or a blank is TRUE (true,yes,...)
      case('F','N')
      lg(i)=.false.         ! assume this is false or no
      case default
...
contains
    ...
    !XX!use ifport, only: isatty
        ...
        select case (get_os_type())
            ...
            case (OS_MACOS)
            os_type =  "OS Type:     macOS"
            case (OS_WINDOWS)
            os_type =  "OS Type:     Windows"
            case (OS_CYGWIN)
            os_type =  "OS Type:     Cygwin"
            case (OS_SOLARIS)
            os_type =  "OS Type:     Solaris"
            case (OS_FREEBSD)
            os_type =  "OS Type:     FreeBSD"
            case (OS_OPENBSD)
            os_type =  "OS Type:     OpenBSD"
            case (OS_UNKNOWN)
            os_type =  "OS Type:     Unknown"
            case default
            os_type =  "OS Type:     UNKNOWN"
        end select
        version_text = [character(len=80) :: &
               ...
               do i=1,ii
                  ...
                  if(line.eq.'')then
                     ...
                     if(verify(line,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_ -').eq.0.and.line(1:1).ne.' ')then
                        ibug=len(esc(line))
                        line='<E><g><bo>'//line//repeat(' ',max(0,80-ibug))
                     else
                        ibug=len(esc(line))
                        line='<E><w>'//line//repeat(' ',max(0,80-ibug))
                     endif
                  endif
...
use fpm_toml, only : toml_serializer
use tomlf_type_table, only : toml_table
implicit none
